...
const contactsNameForTest = ['小怪兽', '斩月', 'Agnes', '西瓜']
...
.on('ready', async dummy => {
  ...
  if (personalContacts.length >= 4) {
    await test('createRoom', async t => {
      ...
      console.log('invite ', contactsForRoomCreate.map(o => o.name()).join(', '), `into room - ${INITIAL_TEST_ROOM_NAME}`)
      roomForTest = await bot.Room.create(contactsForRoomCreate, INITIAL_TEST_ROOM_NAME)
      console.log(`created room id: ${roomForTest.id}`)
      return roomForTest
    })
